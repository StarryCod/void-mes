  ...
  return (
    <div className="fixed inset-0 flex bg-[#13131a] overflow-hidden">
      ...
      <div className={`${activeTarget ? 'hidden md:flex' : 'flex'} w-full md:w-60 bg-[#0f0f14] flex-col border-r border-white/5 shrink-0`}>
        ...
        {/* List */}
        <div className="flex-1 overflow-y-auto px-2 pb-16 md:pb-0">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#0f0f14] border-t border-white/5 flex items-center justify-around z-40">
        <button 
          onClick={() => { setMobileTab('dms'); setActiveView('dms'); setActiveChat(null); setActiveChannel(null); }} 
          className={`flex flex-col items-center justify-center gap-1 px-4 py-2 ${mobileTab === 'dms' ? 'text-purple-400' : 'text-gray-500'}`}
        >
          <MessageSquare className="w-5 h-5" />
          <span className="text-[10px] font-medium">Чаты</span>
        </button>
        <button 
          onClick={() => { setMobileTab('channels'); setActiveView('channels'); setActiveChat(null); setActiveChannel(null); }} 
          className={`flex flex-col items-center justify-center gap-1 px-4 py-2 ${mobileTab === 'channels' ? 'text-purple-400' : 'text-gray-500'}`}
        >
          <Hash className="w-5 h-5" />
          <span className="text-[10px] font-medium">Каналы</span>
        </button>
        <button 
          onClick={() => { setMobileTab('settings'); setShowSettings(true); }} 
          className={`flex flex-col items-center justify-center gap-1 px-4 py-2 ${mobileTab === 'settings' ? 'text-purple-400' : 'text-gray-500'}`}
        >
